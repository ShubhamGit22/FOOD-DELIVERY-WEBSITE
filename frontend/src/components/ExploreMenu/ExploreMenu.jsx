...
import { menu_list } from '../../contentFiles/contentFiles';
                        ...
                        menu_list.map((item, index)=>{
                            return (
                                 <div onClick={() => setCategory(prev => prev===item.menu_name ? "All" : item.menu_name)} key={index} className='explore-menu-list-item'>                                  
                                    <img className={category === item.menu_name ? "active" : ""} src={item.menu_image} alt="img" height="100px" width="100px" />
                                    <p> {item.menu_name} </p>
                                </div>
                                                                                                            //when clicked on any product that product name will be stored in category.  
                            )
                        })
                    }
                </div>
